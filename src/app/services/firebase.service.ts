...
export class FirebaseService {
  ...
  getTableNamesContainingData(tables: Table[]): number[] {
    const out = new Set() as Set<number>;
  ...
  emitTables(tableList: any[]) {
    ...
    const columnsWithData = [] as any[];
    const dataMappedToColumns = [];
    const tablesWithData = [] as Category[];
    const i = 0;
    let item: string;
  ...
  getUniqueListBy = (arr, key) =>
    [...new Map(arr.map((item) => [item[key], item])).values()] as {}[];
