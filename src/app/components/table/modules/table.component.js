...
var TableComponent = /** @class */ (function () {
  function TableComponent(
    ...
    this.groupProperty = "SearchTerms";
        ...
        animations: [
          ...
          animations_1.trigger("translateRationale", [
            ...
            animations_1.transition(
              "closed<=>expanded",
              animations_1.animate("500ms cubic-bezier(0.4, 0.0, 0.2, 1)")
            ),
            animations_1.transition(
              "expanded<=>closed",
              animations_1.animate("500ms cubic-bezier(0.4, 0.0, 0.2, 1)")
          ...
          animations_1.trigger("detailExpand", [
            animations_1.state(
              "collapsed",
              animations_1.style({ height: "0px", minHeight: "0" })
            ),
            animations_1.state("expanded", animations_1.style({ height: "*" })),
            animations_1.transition(
              "expanded <=> collapsed",
              animations_1.animate("225ms cubic-bezier(0.4, 0.0, 0.2, 1)")
            ),
          ]),
          ...
          animations_1.trigger("indicator", [
            animations_1.state(
              "active-asc, asc",
              animations_1.style({ transform: "translateY(-6px)" })
            ),
            // 10px is the height of the sort indicator, minus the width of the pointers
            animations_1.state(
              "active-desc, desc",
              animations_1.style({ transform: "translateY(4px)" })
            ),
            animations_1.transition(
...
var BeersTableDataSource = /** @class */ (function (_super) {
    ...
    set: function (headers) {
      ...
      rxjs_1
        ...
        .pipe(
          ...
          operators_1.map(function (items) {
            return _this._groupTerm
              ? items.filter(function (item) {
                  return item != _this.groupTerm;
                })
              : items;
          })
        )
        .subscribe(function (items) {
          return _this._displayedHeaders.next(items);
        });
    },
    enumerable: false,
    configurable: true,
  ...
  BeersTableDataSource.prototype.convertObservableToBehaviorSubject = function (
    observable
  ) {
    var observed;
  ...
  BeersTableDataSource.prototype._groupingPredicate = function (data) {
    ...
    var _loop_1 = function (header) {
      ...
      if (expanded) {
        dataWithHeaderRows = dataWithHeaderRows.concat(
          [headerRow],
          data.reduce(function (rows, item) {
    ...
    var this_1 = this;
    for (
      var _i = 0, groupHeaders_1 = groupHeaders;
      _i < groupHeaders_1.length;
      _i++
    ) {
  ...
  BeersTableDataSource.prototype._updateChangeSubscription = function () {
    ...
    var groupingParameters = rxjs_1.combineLatest([
      this._group,
      this._groupTerm,
    ]);
    // Watch for base data or filter changes to provide a filtered set of data.
    var filteredData = rxjs_1.combineLatest([dataStream, this._filter]).pipe(
    ...
    var orderedData = rxjs_1.combineLatest([filteredData, sortChange]).pipe(
      operators_1.map(function (_a) {
        var data = _a[0];
        return _this._orderData(data);
      })
    );
    var groupedData = rxjs_1
      .combineLatest([orderedData, groupingParameters])
      .pipe(
        operators_1.map(function (_a) {
          var data = _a[0];
    ...
    (_a = this._renderChangesSubscription) === null || _a === void 0
      ? void 0
      : _a.unsubscribe();
