 ...
 */
import { BaseTableTreeControl } from './base-tree-control.control';
...
export class FlatTreeControl<T, K = T> extends BaseTableTreeControl<T, K> {
  ...
  getDescendants(dataNode: T): T[] {
    ...
    for (
      ...
      this.getLevel(dataNode) < this.getLevel(this.dataNodes[i]);
      i++
    ) {
  ...
  expandAll(): void {
    this.expansionModel.select(
      ...this.dataNodes.map((node) => this._trackByValue(node))
    );
  }
}
